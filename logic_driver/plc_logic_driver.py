...
logging.getLogger("pyrebase").setLevel(logging.DEBUG)  # Enable Firebase SDK debug logging
...
def read_plc_data(plc_ip, parameter):
    try:
        ...
        logging.error(f"Error reading from {plc_ip} for address {parameter}: {e}")
...
def main():
    ...
    while True:
        try:
            ...
            logging.info("Start reading PLC data")
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:  # Limit parallelism to 10 threads
                ...
                for future in futures:
                    key = futures[future]
                    try:
                        parameter, value = future.result(timeout=5)  # Add a timeout
                        all_read_results[key] = value
                    except Exception as e:
                        logging.error(f"Error occurred while retrieving data for {key}: {e}")
            ...
            if output_dict:
                try:
                    ...
                    logging.info(f"Successfully updated Firebase with data")
                except Exception as e:
                    if "429" in str(e):  # Detect throttling explicitly
                        logging.error("Firebase throttling detected (HTTP 429 - Too Many Requests). Adding delay to avoid further throttling.")
            ...
            if time.time() - last_update_time > watchdog_timeout:
                logging.error("No successful Firebase updates within watchdog timeout. Restarting script...")
                break  # Exit loop to restart the script

            logging.info(f"Total time taken for iteration: {time.time() - start_time:.2f} seconds")

            # Add a short delay after Firebase update to avoid throttling
            time.sleep(firebase_delay)

        except Exception as e:
            logging.error(f"Error in main loop: {e}. Continuing execution...")
